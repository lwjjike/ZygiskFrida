...
TMP_MODULE_DIR=/data/local/tmp/xbs.fgg
...
mkdir -p "$TMP_MODULE_DIR/system"
mkdir -p "$TMP_MODULE_DIR/system/lib"
mkdir -p "$TMP_MODULE_DIR/system/lib64"

if [ "$ARCH" = "arm" ] || [ "$ARCH" = "arm64" ]; then
  ui_print "- Extracting arm libraries"
  extract "$ZIPFILE" "gadget/xbs666-arm.so" "$TMP_MODULE_DIR/system/lib/" true
  mv "$TMP_MODULE_DIR/system/lib/xbs666-arm.so" "$TMP_MODULE_DIR/system/lib/libxbs.so"

  ui_print "- Extracting arm64 libraries"
  extract "$ZIPFILE" "gadget/xbs666-arm64.so" "$TMP_MODULE_DIR/system/lib64/" true
  mv "$TMP_MODULE_DIR/system/lib64/xbs666-arm64.so" "$TMP_MODULE_DIR/system/lib64/libxbs.so"

  # -------- added --------
  ui_print "- Extracting libxbs.config.so"
  extract "$ZIPFILE" "gadget/libxbs.config.so" "$TMP_MODULE_DIR/system/lib64/" true
  cp "$TMP_MODULE_DIR/system/lib64/libxbs.config.so" "$TMP_MODULE_DIR/system/lib"
  # -------- added end --------
fi
