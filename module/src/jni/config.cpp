...
static std::optional<target_config> deserialize_target_config(const rapidjson::Value &doc) {
    ...
    // -------- added --------
    auto &inject_mode = doc["inject_mode"];
    if (!inject_mode.IsString()) {
        LOGE("expected config target to have a vaild inject_mode");
        return std::nullopt;
    }
    result.inject_mode = inject_mode.GetString();
    // -------- added end --------
